...
class Shopping_model extends CI_Model{
  ...
  public function save_pro_data($insert_data){
     $res= $this->db->insert('tbl_wishlist',$insert_data); 
...
public function get_cat_subcat_data($child_id){
  $this->db->select("*");
 ...
 function product_cat_data($cid){
  ...
  function product_scat_data($scid){
         $this->db->select("tbl_products.*,tbl_product_attribute.id as attr_id");
         $this->db->from("tbl_products");
          $this->db->join("tbl_product_attribute",'tbl_product_attribute.product_id=tbl_products.id');
         $this->db->group_by("tbl_product_attribute.product_id","asc");
 ...
 function product_list($pid){
   ...
   function cat_product_list($cat_id){
         $this->db->select("*");
         $this->db->from("tbl_products");
         //$this->db->order_by("id","desc");
   ...
   public function get_product_details($cart_id){
    ...
    function Get_related_details($p_id,$attr_id){
         echo"fghfghgh";

   }
  function get_product_gallery_details($p_id,$attr_id){
...
public function bin_data($pid){
       $this->db->select("bin_id");
       $this->db->from("tbl_bin_win_product");
       $this->db->where('tbl_bin_win_product.win_id',$pid);
       $query=$this->db->get();
       //echo $this->db->last_query();
       $res=$query->row();
       return $res;
 ...
 public function get_single_product_sprial_details($pid){
       $this->db->select("tbl_products.*,product_gallery.id as photo_id,product_gallery.image");
       $this->db->from("tbl_products");
       $this->db->join('product_gallery','product_gallery.pdt_id=tbl_products.id');
       $this->db->order_by('product_gallery.id','desc');
