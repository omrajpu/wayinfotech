	...
	public function __construct()
	     {
           ...
           // $this->load->model('../../third_party/common/Crud_model');     
...
public function add_staff()
    {
         ...
         //   $user_roll_id = $this->input->post('id');
          ...
          if($user_roll_id == '') {
                array_push($validation_array,
                    array(
                        "field_name" => "username",
                        "validate_type" => "trim|required|edit_unique[system_admin.username.id.'.$user_roll_id.']",
                        "message" => 'Please enter User Name.'
                    ));
                array_push($validation_array,
                    array(
                        "field_name" => "email",
                        "validate_type" => "trim|required|edit_unique[system_admin.email.id.'.$user_roll_id.']",
          ...
          //  echo $this->db->last_query();
           ...
           $result['save_data_id'] = $save_data_id;
            $result['search_url'] = base_url().'Staff/all_staff';
        ...
        if($user_id){
       $pageData['list_data'] = $this->Crud_model->getCommonData('system_admin', 'id', $user_id);
        }else
        {
          $pageData['list_data']=array();  
        }    
          // print_r( $pageData['list_data']);die;
        $pageData['subview']="add_staff";
